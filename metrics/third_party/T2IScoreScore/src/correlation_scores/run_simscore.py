...
from metrics.third_party.T2IScoreScore.src.correlation_scores.sim_score import SimScorer
...
class CLIPScorer(SimScorer):
    ...
    def calculate_score(self, image, prompt, direct_img=None):
        if not direct_img:
...
class ALIGNScore(SimScorer):
    def __init__(self):
        self.processor = AlignProcessor.from_pretrained(
            "kakaobrain/align-base", cache_dir="<CACHE_DIR>"
        )
    ...
    def calculate_score(self, image, prompt, direct_img=None):
        if not direct_img:
            image = Image.open(image)
        image = image.convert("RGB") #Modified the repo to handle both png and jpg
...
def score(config):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        if os.path.exists(image_file_path):
            ...
            record = {"id": row["id"], "image_id": formatted_image_id, "score": score}
...
def normalize(df):
    df_scores = df["score"]
    scaler = MinMaxScaler()
    df_normalized_scores = pd.DataFrame(
        scaler.fit_transform(df_scores.values.reshape(-1, 1)), columns=["score"]
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate scores for images in a meta-data file."
    )
    parser.add_argument(
        "-m",
        "--model",
        required=True,
        default="clip",
        help="Choose sim score model (clip, blip, align)",
    )
    parser.add_argument(
        "-o",
        "--output",
        required=True,
        default="output/clipscore.csv",
